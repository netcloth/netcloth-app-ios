    ...
    KeyboardReturn
{
    ...
    //MARK:- Public
    /// not sort by msgId, pleast use time server_msg_id:  small at first , big at last
    var messageArray: [CPMessage] = []
    var synManager: GroupMsgSynchronize?
    
    var last_big_msg_id : Int64 = 0
    
    ///last see serverMsg id
    ...
    func findLastOpenMsgId() -> Int64 {
        ...
        let tmpArray = self.messageArray.reversed() //大的第一个
    ...
    func addFakeLocationTip() {
        ...
        //Note: after next once, maybe issue
    ...
    /// 之前显示的定位在底部（-> 中间，两边都能显示了）， 新的lastMsgId ，可能不再是 tmpId
    func _refreshToTop() {
        let tmpId = lastMsgId
        self.reloadTableView()
        var toRow = 0
...
//            if model.server_msg_id == self.synManager?.const_open_msg_id {
//                self.unreadLocationV?.isHidden = true
//            }
        }
    }
